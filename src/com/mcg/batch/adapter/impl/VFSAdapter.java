 ...
 * 
 */
public final class VFSAdapter extends BaseBatchAdapter < ArrayList < VFSResource >> {
 /**
  * Logger to be used by this class
  */
 private static final Logger LOGGER = LoggerFactory
 .getLogger(VFSAdapter.class);
 ...
 public void performOperation(FileOperation operation,
  ...
  boolean isSourceHTTP = HTTP_SCHEME.equals(source.getFileName()
   .getScheme()) || HTTPS_SCHEME.equals(source.getFileName().getScheme());
  long targetFSSize;
  ...
  try {
   ...
   // batchVFSParameters.getSourcePreScript(), source);
   ...
   //
   // scriptOutput = executeCommand(
   // batchVFSParameters.getTargetPreScript(), target);
   // if (LOGGER.isInfoEnabled()) {
   ...
   try {
    if (LOGGER.isDebugEnabled()) {
     LOGGER.debug("Source Adapter URI : " + source.getFileName().getFriendlyURI());
     LOGGER.debug("Source Adapter Path : " + source.getFileName().getPath());
     LOGGER.debug("Source File Name Relative Path : " + batchVFSParameters.getRelativeDirectory());
    }
    srcFileObject = isSourceHTTP ? manager.resolveFile(
     source.getURI(concat(batchVFSParameters.getRelativeDirectory(),
      batchVFSParameters.getFileName())),
     fsSourceOptions) : manager.resolveFile(
     validatenJoinFileURI(source.getFileName().getURI(),
      batchVFSParameters.getRelativeDirectory()),
     fsSourceOptions);
   } catch (FileSystemException e) {
    ThreadContextUtils.addToExecutionContext(BATCH_FAULTY_RESOURCE, source.getResourceAlias());
    throw new RetryableException(
     "exception while performing operation " + operation, e);
   }
   try {
    if (LOGGER.isDebugEnabled()) {
     LOGGER.debug("Target Adapter URI : " + target.getFileName().getFriendlyURI());
     LOGGER.debug("Target Adapter Path : " + target.getFileName().getPath());
     LOGGER.debug("Target File Name Relative Path : " + batchVFSParameters.getTargetRelativePath());
    }
    tgtBaseFileObject = manager.resolveFile(validatenJoinFileURI(target.getFileName()
     .getURI(), batchVFSParameters.getTargetRelativePath()), fsTargetOptions);
   } catch (FileSystemException e) {
    ThreadContextUtils.addToExecutionContext(BATCH_FAULTY_RESOURCE, target.getResourceAlias());
    throw new RetryableException(
     "exception while performing operation " + operation, e);
   }
   sourceFiles = new ArrayList < FileObject > ();
   if (LOGGER.isDebugEnabled()) {
    LOGGER.debug("The source file directory..." + srcFileObject.getName().getURI());
    LOGGER.debug("The target file directory..." + tgtBaseFileObject.getName().getURI());
   }
   if (isSourceHTTP) {
   ...
   /*if (batchVFSParameters.isCheckSource()
   		&& CollectionUtils.isEmpty(sourceFiles)) {
       LOGGER.error(StackUtils.formatException(ThreadContextUtils.getJobExecution().
       		getJobInstance().getJobName(), source.getResourceAlias() ,
       		"The source file check is specified true and no file found matching the input source file.. Terminating the batch...",
       		null));
       ThreadContextUtils.addToExecutionContext(BATCH_FAULTY_RESOURCE, source.getResourceAlias());
   ...
   /**
    * 1030 30 1010 Add the file names as a delimiter separated value to the
    * file.
    */
   if (!sourceFiles.isEmpty()) {
    ...
    for (int i = 0; i < sourceFiles.size(); i++) {
     ...
     if (LOGGER.isInfoEnabled()) {
      LOGGER.info("For Batch Id " + ThreadContextUtils
       .getJobExecutionIdAsString() + " The File Age Strategy is " + (batchVFSParameters.getMinFileAge() > 0));

      LOGGER.info("Minimum File Age : " + batchVFSParameters
       .getMinFileAge());

      LOGGER.info("The source file " + fileObject.getName().getPath() + " was last modified at : " + fileObject.getContent().getLastModifiedTime());
      LOGGER.info("current system time : " + System
       .currentTimeMillis());
        ...
        case RETRY:
         {
          ...
          ThreadContextUtils.addToExecutionContext(BATCH_FAULTY_RESOURCE, target.getResourceAlias());
          throw new RetryableException(
           "Retrying the File transfer as the source file " + fileObject.getName()
           .getPath() + " has a last modified time less than the Minimum File Age limit and the the action is retry.");
         }
        case TERMINATE:
         {
          /*LOGGER.error(StackUtils.formatException(ThreadContextUtils.getJobExecution().
          ...
          ThreadContextUtils.addToExecutionContext(BATCH_FAULTY_RESOURCE, source.getResourceAlias());
          throw new NonRetryableExecption(
           "Terminating the Batch  as the source file " + fileObject.getName()
           .getPath() + " has a last modified time less than the Minimum File Age limit and the the action is terminate. ");
         }
        case SKIP:
         {
          if (LOGGER.isInfoEnabled()) {
           LOGGER.info("{Execution =" + ThreadContextUtils
            .getJobExecutionIdAsString() + " }, as per the batch configuration the file " + fileObject.getName().getPath() + " will be skipped as the " + "last modified time less than the Minimum File Age limit");
           ThreadContextUtils
     ...
     targetFilePaths.append(tgtBaseFileObject.getName()
      .getPath() + FileName.SEPARATOR_CHAR + fileObject.getName().getBaseName());
   ...
   if (batchVFSParameters.getSourcePreScript() != null) {
    ...
    scriptOutput = executeCommand(
     batchVFSParameters.getSourcePreScript() + sourceFilePaths.toString(), source);
    if (LOGGER.isInfoEnabled()) {
     LOGGER.info(concat("Executed the  source pre script ",
      batchVFSParameters.getSourcePreScript() + sourceFilePaths.toString(),
      " and the output is ", scriptOutput));
    }
   }
   if (batchVFSParameters.getTargetPreScript() != null) {
    ...
    scriptOutput = executeCommand(
     batchVFSParameters.getTargetPreScript() + targetFilePaths.toString(), target);
    if (LOGGER.isInfoEnabled()) {
     LOGGER.info(concat("Executed the  target  pre script ",
      batchVFSParameters.getTargetPreScript() + targetFilePaths.toString(),
      " and the output is ", scriptOutput));
    }
   }
   ...
   //To check the threshold value is lesser than or equal to the Filesystem used space

   if (target.getBatchVFSThreshold() > 0) {
    threshold = target.getBatchVFSThreshold();
    if (threshold <= capacity) {
     String hostname = null;
     Long jobExecutionId = ThreadContextUtils.getJobExecutionId();

     if (LOCAL_FS_SCHEME.equals(target.getFileName().getScheme())) {
      hostname = "Localhost";

     } else if (SFTP_SCHEME
      .equals(target.getFileName().getScheme()) || FTP_SCHEME.equals(target.getFileName().getScheme())) {
      hostname = ((GenericFileName) target.getFileName()).getHostName();
     }

     String message = "The Filesystem usage has reached the threshold of " + threshold + "% for the server " + hostname;
   ...
   if (targetFSSize < totalSourceSize) {
    /*LOGGER.error(StackUtils.formatException(ThreadContextUtils.getJobExecution().
			    		getJobInstance().getJobName(), target.getResourceAlias() ,
   ...
   for (int j = 0; j < sourceFiles.size(); j++) {
    ...
    if (batchVFSParameters.isSingleFile() && StringUtils.isNotBlank(batchVFSParameters.getTargetFileName())) {
     targetFileName = batchVFSParameters.getTargetFileName();
    }
    tgtFile = tgtBaseFileObject.resolveFile(validatenJoinFileURI(tgtBaseFileObject
     .getName().getPath(), FileName.SEPARATOR_CHAR + targetFileName));
    LOGGER.debug("Target file name BASE:" + fileObject.getName().getBaseName());
    LOGGER.debug("Target file name EXT:" + fileObject.getName().getExtension());
    try {
     String ext = fileObject.getName().getExtension();
     if (!StringHelper.EMPTY_STRING.equals(ext)) {
      targetFileNametemp = targetFileName.substring(0, targetFileName.lastIndexOf(ext) - 1);
     }
    } catch (Exception e) {
     LOGGER.debug("Exception on temp target file generation");
    }
    if (tgtFile.exists() && batchVFSParameters.getTransferType() == NO_REPLACE) {
     /*LOGGER.error(StackUtils.formatException(ThreadContextUtils.getJobExecution().
				    		getJobInstance().getJobName(), target.getResourceAlias() ,
				    		"The file "
     ...
     ThreadContextUtils.addToExecutionContext(BATCH_FAULTY_RESOURCE, target.getResourceAlias());
     throw new NonRetryableExecption(
      "The file " + tgtFile.getName().getPath() + " already exists and the batch is initiated with NO_REPLACE mode...");
    } else if (tgtFile.exists() && batchVFSParameters.getTransferType() == TransferType.COPY_OF) {
     ...
     for (int i = 1;; i++) {
      ...
      tgtFile = tgtBaseFileObject
       .resolveFile(validatenJoinFileURI(tgtBaseFileObject.getName()
        .getPath(),
        FileName.SEPARATOR_CHAR + newFileName + "_" + i + ext));
    ...
    tgtFile = tgtBaseFileObject.resolveFile(validatenJoinFileURI(tgtBaseFileObject
     .getName().getPath(),
     FileName.SEPARATOR_CHAR + targetFileNametemp + ".part"));
    ...
    if (tgtFile.exists()) {
     ...
     for (int i = 1;; i++) {
      tgtFile = tgtBaseFileObject.resolveFile(validatenJoinFileURI(tgtBaseFileObject
       .getName().getPath(),
       FileName.SEPARATOR_CHAR + targetFileNametemp + "_" + i + ".part"));
      if (!tgtFile.exists()) {
       break;
      }
     }
    ...
    if ((batchVFSParameters.getTransferType() == APPEND && tgtFileclone.exists())) {
     tgtFileclone.moveTo(tgtFile); //Change the target file to part file name, because get content of target file.
     os = tgtFile.getContent().getOutputStream(
      batchVFSParameters.getTransferType() == APPEND);
    } else {
     os = tgtFile.getContent().getOutputStream(
      batchVFSParameters.getTransferType() == APPEND && tgtFile.exists());
    }
    ...
    //After completion of downloading file, will goto rename the actual copy of file, in between gap any batch creates same file name,
    //So avoid the overwrite file, create new file name
    if (batchVFSParameters.getTransferType() == TransferType.COPY_OF) {
     tgtFileclone = tgtBaseFileObject.resolveFile(validatenJoinFileURI(tgtBaseFileObject
      .getName().getPath(),
    ...
    if (LOGGER.isDebugEnabled()) {
     LOGGER.debug("Transfer completed ... Total bytes transfered is " + transferedBytes);
    }
   ...
   if (batchVFSParameters.getSourcePostScript() != null) {
    scriptOutput = executeCommand(
     batchVFSParameters.getSourcePostScript() + sourceFilePaths.toString(), source);
    if (LOGGER.isInfoEnabled()) {
     LOGGER.info(concat("Executed the  source post script ",
      batchVFSParameters.getSourcePostScript() + sourceFilePaths.toString(),
      " and the output is ", scriptOutput));
    }
   }
   if (batchVFSParameters.getTargetPostScript() != null) {
    scriptOutput = executeCommand(
     batchVFSParameters.getTargetPostScript() + targetFilePaths.toString(), target);
    if (LOGGER.isInfoEnabled()) {
     LOGGER.info(concat("Executed the  target  post script ",
      batchVFSParameters.getTargetPostScript() + targetFilePaths.toString(),
      " and the output is ", scriptOutput));
    }
   }
 ...
 throws AdapterException {
  if (LOGGER.isTraceEnabled()) {
   LOGGER.trace("VFSAdapter.executeScript() started and the command is " + command);
  }
  JSch jsch = null;
  ...
  int exitCode = -1;
  try {
   if (SFTP_SCHEME.equals(vfsResource.getFileName().getScheme()) || FTP_SCHEME.equals(vfsResource.getFileName().getScheme())) {
    jsch = new JSch();
    ...
    channel.setCommand(command); in = channel.getInputStream();
    channel.connect();
    tmp = new byte[KB];
    ...
    if (LOGGER.isDebugEnabled()) {
     LOGGER.debug("The Command has been executed and the output is " + output.toString());
    }
    if (exitCode > 0) {
     ThreadContextUtils.addToExecutionContext(BATCH_FAULTY_RESOURCE, vfsResource.getResourceAlias());
     throw new NonRetryableExecption(concat(
      "Unable to execute the command :", command) + (output.length() > 0 ? concat(". Output : ", output.toString()) : ""));
    }
 ...
 private static int getFileTypeFSUsedPercentage(File file) {
  ...
  double used = (file.getTotalSpace() - file.getFreeSpace() / file.getTotalSpace()) * 100;
  fileUsedPercentage = (int) Math.round(used);
  return fileUsedPercentage;
 }
 private static long getFSFreeSpace(VFSResource vfsResource, BatchVFSParameters batchVFSParameters)
 throws AdapterException {
  if (LOGGER.isTraceEnabled()) {
  ...
  try {
   if (LOCAL_FS_SCHEME.equals(vfsResource.getFileName().getScheme())) {
    ...
    if (LOGGER.isDebugEnabled()) {
     LOGGER.debug("The local file path is  " + file.getAbsolutePath());
    }
    if (!file.exists()) {
     try {
      file.mkdir();
    ...
    .equals(vfsResource.getFileName().getScheme()) || FTP_SCHEME.equals(vfsResource.getFileName().getScheme())) {
    output = executeCommand(
     concat(DISK_SPACE_COMMAND, validatenJoinFileURI(vfsResource.getFileName()
      .getPath(), batchVFSParameters.getTargetRelativePath())), vfsResource);
    if (isNotEmpty(output)) {
     ...
     try {
      ...
      capacity = Integer.valueOf(capacityString.substring(0, capacityString.length() - 1));

     } catch (NumberFormatException e) {
      errorMessage = "Invalid Number specified...";
      putWarningForExecutionId(executionId, errorMessage);
      if (LOGGER.isWarnEnabled()) {
       LOGGER.warn("Invalid Number specified..." + output.toString());
      }
     } catch (NullPointerException e) {
      errorMessage = "Invalid Format specified...";
      putWarningForExecutionId(executionId, errorMessage);
      if (LOGGER.isWarnEnabled()) {
       LOGGER.warn("Invalid Format expected..." + output.toString());
      }
     }
    } else {
   ...
   if (LOGGER.isTraceEnabled()) {
    LOGGER.trace("VFSAdapter.getFSFreeSpace() capacity  -> " + capacity);
   }
 ...
 throws Exception {
  try {
   ...
   if (LOGGER.isDebugEnabled()) {
    LOGGER.debug("Checking whether file has got Child Files : " + file.getType().hasChildren());
   }
   if (file.getType().hasChildren() && selector.traverseDescendents(fileInfo)) {
    ...
    FileObject[] children = FileObjectSorter.sortAndGetChildren(file,
     sortOrder, sortBy);
    if (LOGGER.isDebugEnabled()) {
    ...
    for (int i = 0; i < children.length; i++) {
     ...
     traverse(fileInfo, selector, depthwise, selected, sortOrder,
      sortBy);
    }
   ...
   if (LOGGER.isDebugEnabled()) {
    LOGGER.debug("Checking whether to Include the file : #####" + fileInfo.getFile().getName().getBaseName());
   }
 ...
 public static void findFiles(FileObject fileObject, FileSelector selector,
  ...
  SortBy sortBy) throws FileSystemException {
  try {
   if (LOGGER.isDebugEnabled()) {
    LOGGER.debug("Inside findFiles looking for " + fileObject.getName().getURI());
   }
   if (LOGGER.isDebugEnabled()) {
  ...
  */
 public static void putWarningForExecutionId(final Long jobExecutionId, final String message) {
  try {
   if (LOGGER.isTraceEnabled()) {
    LOGGER.trace("VFSAdapter.putWarningForExecutionId() started ");
   }
   CACHE_FACTORY.getCache().appendToMapEntry(BATCH_EXECUTION_WARNINGS, jobExecutionId, message);
  } finally {
   if (LOGGER.isTraceEnabled()) {
 ...
 public String getWarningForExecutionId(final Long jobExecutionId) {
  ...
  try {
   ...
   message = (String) CACHE_FACTORY.getCache().getFromMap(BATCH_EXECUTION_WARNINGS, jobExecutionId);
 ...
 public void updateFileFilterStatustoContext(List < FileObject > files, VFSResource resource) throws RetryableException {
  ...
  if (ThreadContextUtils.getExecutionContext().containsKey(FILE_COUNT_FILTER_STATUS)) {
   fileCountFilterStatus = (boolean) ThreadContextUtils.getExecutionContext().get(FILE_COUNT_FILTER_STATUS);
  }

  if (ThreadContextUtils.getExecutionContext().containsKey(FILE_NAME_FILTER_STATUS)) {
   fileNameFilterStatus = (boolean) ThreadContextUtils.getExecutionContext().get(FILE_NAME_FILTER_STATUS);
  }

  if (ThreadContextUtils.getExecutionContext().containsKey(FILE_SIZE_FILTER_STATUS)) {
   fileSizeFilterStatus = (boolean) ThreadContextUtils.getExecutionContext().get(FILE_SIZE_FILTER_STATUS);
  }

  if (files.isEmpty() && !fileNameFilterStatus) {
   ThreadContextUtils.addToExecutionContext(BATCH_FAULTY_RESOURCE, resource.getResourceAlias());
   throw new RetryableException(
